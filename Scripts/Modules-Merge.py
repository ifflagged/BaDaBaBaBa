...
def merge_modules(input_file, output_type, module_urls):
    ...
    for module_url in module_urls:
        ...
        # Initialize separate sets for storing hosts
        append_hosts = set()
        insert_hosts = set()
        disabled_hosts = set()
        ...
        if mitm_section:
            for line in mitm_section:
                if line.lower().startswith("hostname = %append%"):
                    ...
                    disabled_hosts.update(host.strip() for host in disabled_hosts.split(",") if host.strip())
        ...
        for section_name, content_list in module_content.items():
            if content_list and any(line.strip() for line in content_list):
                ...
                if section_name == "MITM":
                    if output_type == 'sgmodule':
                        combined_mitmh = "hostname = %APPEND% " + ", ".join(sorted(append_hosts)) if append_hosts else ""
                        output_file.write(combined_mitmh + "\n")
                        if disabled_hosts:
                            disabled_line = "hostname-disabled = %INSERT% " + ", ".join(sorted(disabled_hosts))
                            output_file.write(disabled_line + "\n")
                    else:
                        if append_hosts or insert_hosts:
                            output_file.write("hostname = " + ", ".join(sorted(append_hosts | insert_hosts)) + "\n")
                else:
                    output_file.write("\n".join(content_list) + "\n")
                output_file.write("\n")
