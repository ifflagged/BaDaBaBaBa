...
$comment" \
    ...
    -e '/^\^http.*?(?!.*script-path)(reject|302|307|header-del.*|header-add.*|header-replace.*|header-replace-regex.*|request-header.*|request-body.*|response-header.*|response-body.*)/ { s/^\(^\^http[^ ]*\)\( .*$/\1 - reject/; }' \
